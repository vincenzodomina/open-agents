...
import { getGitHubAccount } from "./github-accounts";
...
function resolvePullRequestAppBaseUrl(appBaseUrl?: string): string | null {
  const candidates = [
    appBaseUrl,
...
async function resolvePullRequestContextSection(params: {
  sessionId: string;
  appBaseUrl?: string;
}): Promise<string> {
...
function appendPullRequestContextSection(
  body: string,
...
export type GeneratePullRequestContentParams = {
  sandbox: Sandbox;
  sessionId: string;
  sessionTitle: string;
  baseBranch: string;
